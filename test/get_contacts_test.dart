    ...
    test(
      'get contacts',
        () async {
          ...
          String tomasUserName = "${prefix}Tomas${tomas.accountId.substring(0, 5)}";
          ...
          String platonUserName = "${prefix}Platon${platon.accountId.substring(0, 5)}";
          ...
          kc2Service.newUserCallback = (tx) async {
            ...
            switch (counter) {
              case 1:
                ...
                await kc2Service.newUser(tomas.accountId, tomasUserName, tomasPhoneNumber);
              case 2:
                kc2Service.setKeyring(tor.keyring);
                kc2Service.subscribeToAccount(tor.accountId);
                await kc2Service.newUser(tor.accountId, torUserName, torPhoneNumber);
              case 3:
                kc2Service.setKeyring(platon.keyring);
                kc2Service.subscribeToAccount(platon.accountId);
                await kc2Service.newUser(platon.accountId, platonUserName, platonPhoneNumber);
              // When all users created
              case 4:
                final contacts = await kc2Service.getContacts('${prefix}To');
                expect(contacts.any((contact) => contact.userName == tomUserName), isTrue);
                expect(contacts.any((contact) => contact.userName == tomasUserName), isTrue);
                expect(contacts.any((contact) => contact.userName == torUserName), isTrue);
                expect(contacts.any((contacts) => contacts.userName == platonUserName), isFalse);
    ...
    test(
        'pagination',
          ...
          kc2Service.newUserCallback = (tx) async {
            ...
            // When all users created
              case 4:
                final contacts = await kc2Service.getContacts(prefix, fromIndex: 1, limit: 1);
